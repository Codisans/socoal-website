...
export function ContactForm({ className, ...props }: React.ComponentPropsWithoutRef<'form'>) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        emailjs
            .sendForm('service_e4cjy1e', 'template_jslum75', form.current, {
                publicKey: 'okublB-mG-QsMO76O',
            })
            .then(
                () => {
                    setIsSubmitted(true);
            ...
            <div className={clsx(className, 'py-8 text-center')}>
                <div className="mb-4">
                    <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-zinc-100 dark:bg-zinc-800">
                        <svg className="h-8 w-8 text-zinc-600 dark:text-zinc-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                        </svg>
                    </div>
                    <h3 className="mb-2 text-lg font-semibold text-zinc-950 dark:text-white">¡Mensaje enviado!</h3>
                    <p className="text-zinc-600 dark:text-zinc-400">Gracias por contactarnos. Te responderemos lo antes posible.</p>
                </div>
    ...
    return (
        <form {...props} className={clsx(className)} onSubmit={handleSubmit} ref={form}>
            ...
            <div className="mt-10 flex items-center gap-4">
                ...
                <Button type="submit" disabled={isSubmitting}>
                    <span className="w-28"> {isSubmitting ? 'Enviando...' : 'Enviar mensaje'}</span>
                </Button>
            </div>
        </form>
    );
...
function TextInput({ label, invalid, ...props }: React.ComponentPropsWithoutRef<'input'> & { label: string; invalid?: boolean }) {
    let id = useId();
            ...
            <input
                ...
                className={clsx(
                    ...
                    invalid
                        ...
                        : 'border-zinc-200 text-zinc-950 focus:ring-theme/80',
                )}
            />
            <label
                htmlFor={id}
...
function TextArea({ label, invalid, ...props }: React.ComponentPropsWithoutRef<'textarea'> & { label: string; invalid?: boolean }) {
    let id = useId();
